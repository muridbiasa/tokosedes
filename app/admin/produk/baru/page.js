...
function NewProductPageContent() {
  ...
  const { storeId, settings } = useStoreSettings(requestedStoreId || undefined);
  ...
  // Muat daftar produk VIA SERVER (Admin SDK) — kebal terhadap Security Rules
  // client yang membatasi read stores/{id}/products dari browser.
  function loadProducts() {
    if (!storeId) return;
    setLoadingProducts(true);
    setProductsError("");
    fetch(`/api/admin/product/list?storeId=${encodeURIComponent(storeId)}`)
      .then(async (res) => {
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Gagal memuat produk");
        setProducts((data.products || []).map((d) => productFromDoc(d.productId, d)));
      })
  ...
  async function handleSave(finalProduct, finalFields) {
    ...
    try {
      ...
      // bukan membuat produk duplikat baru. Refresh daftar agar judul ikut baru.
