...
import { uploadProductImage } from "@/lib/productMedia";
...
export default function VariantManager({ productName, variants = [], onChange, storeId }) {
  ...
  async function handleImage(index, file) {
    ...
    try { update(index, { image_url: await uploadProductImage(file, storeId, "draft") }); } catch (error) { alert(error.message); }
  }
  return <div className="flex flex-col gap-3">
    ...
    <div className="flex flex-col gap-2">{variants.map((v, i) => <div key={v.sku || i} className="flex flex-wrap items-center gap-3 rounded-md border border-dashed border-[var(--line)] bg-[var(--paper)] p-3">
      ...
      <label className="flex cursor-pointer items-center gap-1 rounded border border-[var(--line)] px-2 py-1 text-xs"><Upload data-icon="inline-start" /> Gambar<input type="file" accept="image/*" className="sr-only" onChange={(e) => handleImage(i, e.target.files?.[0])} /></label>
