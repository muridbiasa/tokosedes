...
import { Check, Download, ExternalLink, Plus, Search, Store, Trash2, TrendingUp, Wallet, X } from "lucide-react";
...
function AdminDashboardContent() {
  ...
  useEffect(() => subscribeProfiles(setProfiles, (error) => {
    console.error("[dashboard] Failed to load store profiles:", error);
  ...
  useEffect(() => subscribeActiveProfile(setActiveId, (error) => {
    console.error("[dashboard] Failed to load active profile:", error);
  ...
  async function handleCreate(event) {
    ...
    try {
      // Buat profil lalu langsung jadikan satu-satunya toko aktif:
      // semua profil lain otomatis dinonaktifkan.
      const newId = await createProfile(name);
      await activateProfileById(newId);
      setCustomName(""); setShowCreate(false);
    } catch (error) {
      console.error("[dashboard] Failed to create store profile:", error);
  ...
  /**
   * MODEL SATU-TOKO-AKTIF: menyalakan satu toko otomatis mematikan yang lain.
   * `enabled` kini berarti "inilah toko yang sedang dipakai" — tidak ada lagi
   * dua konsep tercampur (enabled vs aktif) yang membingungkan.
   */
  async function activateProfileById(targetId) {
    const others = profiles
      .filter((p) => p.id !== targetId && p.enabled !== false);
    await Promise.all(others.map((p) => updateProfile(p.id, { enabled: false })));
    await updateProfile(targetId, { enabled: true });
  ...
  async function activateProfile(profile) {
    ...
    try {
      ...
      console.error("[dashboard] Failed to activate store:", error);
      setProfilesError(error?.code === "permission-denied" ? "Akses ditolak Firebase. Login admin untuk mengganti toko aktif." : "Gagal mengganti toko aktif.");
    } finally { setSaving(false); }
  }
  ...
  async function deleteProfile(profile) {
    ...
    const productNote = "Katalog produknya juga akan TERHAPUS permanen. Pesanan lama tetap tersimpan sebagai arsip.";
    if (!window.confirm(`Hapus toko "${name}"?\n\n${productNote}`)) return;
    if (!window.confirm(`Konfirmasi terakhir: yakin menghapus "${name}" secara permanen?`)) return;
    setDeletingId(profile.id); setProfilesError("");
    try {
      const res = await fetch("/api/admin/store/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ storeId: profile.id }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Gagal menghapus toko");
      // Profil & pointer aktif diperbarui otomatis lewat onSnapshot.
    } catch (error) {
      console.error("[dashboard] Failed to delete store:", error);
  ...
  return <div className="min-h-screen bg-[var(--canvas)] text-[var(--ink)]">
    ...
    <main className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-6">
      ...
      <section>
        <div className="mb-3 flex items-end justify-between"><div><p className="text-xs uppercase tracking-[0.05em] text-[var(--muted)]">Profil toko</p><h2 className="font-display text-xl font-semibold">Pilih workspace aktif</h2><p className="mt-1 text-xs text-[var(--muted)]">Hanya satu toko yang bisa aktif — menyalakan toko lain otomatis mematikan yang sebelumnya.</p></div><span className="text-xs text-[var(--muted)]">{profiles.length} profil</span></div>
        {profiles.length === 0 ? (
          <div className="flex min-h-56 flex-col items-center justify-center rounded-lg border border-dashed border-[var(--line)] bg-[var(--paper)] p-8 text-center"><div className="mb-4 flex size-12 items-center justify-center rounded-md bg-[var(--canvas)]"><Store size={22} /></div><h3 className="font-display text-lg font-semibold">Belum ada toko</h3><p className="mt-2 max-w-sm text-sm leading-6 text-[var(--muted)]">Buat store profile pertama untuk mulai mengatur katalog, checkout, dan analytics.</p><button type="button" onClick={() => setShowCreate(true)} className="mt-5 flex items-center gap-2 rounded-md bg-[var(--ink)] px-4 py-2 text-sm font-semibold text-[var(--paper)]"><Plus size={16} /> Buat toko pertama</button></div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {profiles.map((profile) => {
              const isActive = profile.id === activeId;
              return (
                <article key={profile.id} className={`rounded-lg border bg-[var(--paper)] p-4 transition-colors ${isActive ? "border-[var(--pine)] shadow-sm ring-1 ring-[var(--pine)]/30" : "border-[var(--line)]"}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className={`flex size-10 items-center justify-center rounded-md ${isActive ? "bg-[var(--pine)]/10 text-[var(--pine)]" : "bg-[var(--canvas)]"}`}><Store size={18} /></div>
                      <div>
                        <div className="flex flex-col gap-1">
                          <Link href={`/admin/orders?storeId=${encodeURIComponent(profile.id)}`} className="font-semibold underline-offset-4 hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--marigold)]">{profile.name}</Link>
                          <Link href={`/admin/produk/baru?storeId=${encodeURIComponent(profile.id)}`} className="text-xs text-[var(--muted)] underline-offset-4 hover:underline">Kelola toko</Link>
                        </div>
                        <p className="font-mono text-[11px] text-[var(--muted)]">{profile.id}</p>
                      </div>
                    </div>
                    {isActive ? (
                      <span className="flex shrink-0 items-center gap-1 rounded-full bg-[var(--pine)]/10 px-2.5 py-1 text-[11px] font-semibold text-[var(--pine)]"><Check size={13} /> Aktif</span>
                    ) : (
                      <button type="button" disabled={saving || deletingId !== null} onClick={() => activateProfile(profile)} title="Jadikan satu-satunya toko aktif" className="shrink-0 rounded-full border border-[var(--line)] px-2.5 py-1 text-[11px] font-semibold hover:border-[var(--pine)] hover:text-[var(--pine)] disabled:opacity-40">Jadikan aktif</button>
                    )}
                  </div>
                  <div className="mt-4 flex items-center justify-between border-t border-dashed border-[var(--line)] pt-3">
                    <span className={`text-xs ${isActive ? "font-medium text-[var(--pine)]" : "text-[var(--muted)]"}`}>{isActive ? "Toko ini sedang dibuka pembeli" : "Nonaktif — klik Jadikan aktif untuk memakai"}</span>
                    <button type="button" disabled={deletingId !== null} onClick={() => deleteProfile(profile)} aria-label={`Hapus toko ${profile.name}`} title="Hapus toko beserta katalognya" className="flex items-center gap-1 rounded-md px-2 py-1 text-xs font-semibold text-[var(--brick)] hover:bg-[var(--brick)]/10 disabled:opacity-40"><Trash2 size={13} />{deletingId === profile.id ? "Menghapus…" : "Hapus"}</button>
                  </div>
                </article>
              );
            })}
